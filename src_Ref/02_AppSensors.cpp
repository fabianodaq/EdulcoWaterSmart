...
unsigned long SensSch_lastUpdate = 0;
uint8_t SensSch_Idx = 0;
...
void Sens_InitSens()
{
  ...
  DS18B20Sensors.begin();       //init DS18   //Serial.print("DS18B20 detected: ");   //Serial.println(DS18B20Sensors.getDeviceCount());
...
void Sens_HanldeSens() {
    ...
    unsigned long Sens_currentMillis = millis();

    if (Sens_currentMillis - SensSch_lastUpdate >= SENS_TIME_INTERVAL) {
        SensSch_lastUpdate = Sens_currentMillis;
        ...
        else if (SensSch_Idx == 1)
        {
          Sens_DS18_Get();
          s_Ctrl[1].ActReadVal = SensDs18.value;
          
          unsigned int tempint = Sens_Ntc_Get();

          if (s_Ctrl[2].type == _NTC){
            s_Ctrl[2].ActReadVal = tempint;

          } else if (s_Ctrl[2].type == _DIFF){
            s_Ctrl[2].ActReadVal = tempint - s_Ctrl[1].ActReadVal;

          } else if (s_Ctrl[2].type == _AVG){
            s_Ctrl[2].ActReadVal = (s_Ctrl[1].ActReadVal + tempint)/2;

          }

        } 
        else if (SensSch_Idx == 2)
        {
          if ((s_Ctrl[2].type == _EC) || (s_Ctrl[2].type == _TDS) || (s_Ctrl[2].type == _SAL))
          {
            Sens_calc_Ec();
          }

        } 
        else if (SensSch_Idx == 3)
        {
          Sens_calc_PHx100_Orp(&SensPhRx[1], MCP342x::channel4, 3);

        }

        SensSch_Idx++;
        if (SensSch_Idx > SENSORS_MAX_IDX) {
          SensSch_Idx = 0; // Reset ciclo
